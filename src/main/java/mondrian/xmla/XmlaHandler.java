...
public class XmlaHandler {
    ...
    private void execute(XmlaRequest request, XmlaResponse response) throws XmlaException {
        ...
        try {
            ...
            boolean rowset = request.isDrillThrough()
                    || Format.Tabular.name().equals(request.getProperties().get(PropertyDefinition.Format.name()));
            writer.startElement("root", "xmlns",
                    result == null ? NS_XMLA_EMPTY : rowset ? NS_XMLA_ROWSET : NS_XMLA_MDDATASET, "xmlns:xsi", NS_XSI,
    ...
    private QueryResult executeDrillThroughQuery(XmlaRequest request) throws XmlaException {
        ...
        try {
            ...
            resultSet = extra.executeDrillthrough(statement, request.getStatement(), advanced, tabFields, rowCountSlot);
        ...
        public TabularRowSet(Map<String, List<String>> tableFieldMap, List<String> tableList) {
            for (String tableName : tableList) {
                ...
                for (String fieldName : fieldNames) {
                    columns.add(new Column(tableName + "." + fieldName, Types.VARCHAR, // don't know the real type
                            0));
                }
            }
        ...
        try {
            ...
            try {
                ...
                if (format == Format.Tabular) {
                    ...
                    dataSet = new MDDataSet_Multidimensional(extra, cellSet,
                            content != Content.DataIncludeDefaultSlicer,
    ...
    public static abstract class MDDataSet implements QueryResult {
        ...
        protected static final List<Property> cellProps = Arrays.asList(rename(StandardCellProperty.VALUE, "Value"),
                rename(StandardCellProperty.FORMATTED_VALUE, "FmtValue"),
                rename(StandardCellProperty.FORMAT_STRING, "FormatString"));

        protected static final List<StandardCellProperty> cellPropLongs = Arrays.asList(StandardCellProperty.VALUE,
    ...
    public static class MDDataSet_Multidimensional extends MDDataSet {
        ...
        public MDDataSet_Multidimensional(XmlaExtra extra, CellSet cellSet, boolean omitDefaultSlicerInfo,
                boolean json) throws SQLException {
            super(cellSet);
...
//                            .getPropertyValue(StandardMemberProperty.CHILDREN_CARDINALITY);
                    Integer childrenCard = DEFAULT_CARDINALITY;
    ...
    public static class XmlaExtraImpl implements XmlaExtra {
        ...
        public String getPropertyValue(PropertyDefinition propertyDefinition) {
            if (propertyDefinition.name().equals("Catalog")) {
                String currentCatalog = XmlaRequestContext.localContext.get().currentCatalog;
